...
def test_normalize_tax_id():
    val, status, warnings = normalize_tax_id("0101234567")
    assert val == "0101234567"
    assert status == "valid"

    val_dash, status, warnings = normalize_tax_id("0101234567-001")
...
def test_parse_date():
    d1, s1, _ = parse_date("15/08/2026")
    assert d1 == "2026-08-15"

    d2, s2, _ = parse_date("Ngày 15 tháng 08 năm 2026")
    assert d2 == "2026-08-15"

    d3, s3, _ = parse_date("2026-08-15")
